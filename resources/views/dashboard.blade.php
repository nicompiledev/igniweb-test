                ...
                <div class="p-6 text-gray-900">
                    ...
                    @else
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @foreach($reservations as $reservation)
                                        <tr id="reservation-{{ $reservation->id }}" class="min-h-[48px]">
                                            <td class="px-4 py-2">{{ $reservation->book->title ?? 'No Title' }}</td>
                                            <td class="px-4 py-2">{{ $reservation->book->author ?? 'Unknown Author' }}</td>
                                            <td class="px-4 py-2">{{ $reservation->book->category ?? 'Unknown Category' }}</td>
                                            <td class="px-4 py-2">{{ $reservation->start_date ? \Carbon\Carbon::parse($reservation->start_date)->format('Y-m-d') : 'N/A' }}</td>
                                            <td class="px-4 py-2">{{ $reservation->end_date ? \Carbon\Carbon::parse($reservation->end_date)->format('Y-m-d') : 'N/A' }}</td>
                                            <td class="px-4 py-2">
                        ...
                        <div class="sm:hidden space-y-4">
                            @foreach($reservations as $reservation)
                                <div id="reservation-card-{{ $reservation->id }}" class="bg-gray-50 shadow-md rounded-lg p-4">
                                    <h5 class="text-lg font-semibold">{{ $reservation->book->title ?? 'No Title' }}</h5>
                                    <p class="text-sm text-gray-600">{{ __('Author: ') . ($reservation->book->author ?? 'Unknown Author') }}</p>
                                    <p class="text-sm text-gray-600">{{ __('Category: ') . ($reservation->book->category ?? 'Unknown Category') }}</p>
                                    <p class="text-sm text-gray-600">{{ __('Start Date: ') . ($reservation->start_date ? \Carbon\Carbon::parse($reservation->start_date)->format('Y-m-d') : 'N/A') }}</p>
                                    <p class="text-sm text-gray-600">{{ __('End Date: ') . ($reservation->end_date ? \Carbon\Carbon::parse($reservation->end_date)->format('Y-m-d') : 'N/A') }}</p>
                                    <div class="mt-2">
                                        <button type="button" class="text-red-600 hover:text-red-900 text-sm" onclick="openModal({{ $reservation->id }})">
                                            {{ __('Delete') }}
                                        </button>
                                    </div>
                                </div>
